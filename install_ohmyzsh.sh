  ...
  if ! command_exists autojump; then
    if user_can_sudo; then
      if command_exists apt; then
        ...
        echo "Only apt and yum are supported."
        return 1
      fi
    else
      fmt_error "You don't have sudo privileges. Please install autojump manually."
      return 1
    fi
  fi
  return 0
...
setup_plugins() {
  # zsh-auto-suggestions, zsh-syntax-highlighting
  
  echo "Installing zsh-auto-suggestions, zsh-syntax-highlighting..."
  ...
  ZSH_PLUGINS="git zsh-syntax-highlighting zsh-autosuggestions"
  ...
  printf '%sDo you want to install autojump? [Y/n]%s ' "$FMT_YELLOW" "$FMT_RESET"
  read -r opt
  case $opt in
    y*|Y*|"") setup_autojump && ZSH_PLUGINS="$ZSH_PLUGINS autojump" ;;
    n*|N*) echo "Don't install autojump.";;
